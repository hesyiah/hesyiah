...
def extract_imsi_contacts(base_path):
    ...
    return [("IMSI (用户识别码)", f"✅ {matches[0]}" if matches else "⚠️ 未找到", os.path.basename(full_path))]
...
def extract_wifi_info(base_path):
    """
    模块4: Wi-Fi 配置信息
    目标: MAC地址, SSID (Wi-Fi名称)
    路径: misc\wifi\WifiConfigStore.xml
    """
    ...
    # 1. 提取 SSID (Wi-Fi名称)
    # 常见格式: &quot;MyWiFi&quot; 或者 "MyWiFi"
    # 使用正则非贪婪匹配提取引号内的内容
    ssid_matches = re.findall(r'&quot;(.*?)&quot;', content)
    if not ssid_matches:
        # 尝试匹配 <string name="SSID">"xxx"</string>
    ...
    if unique_ssids:
        ...
        results.append(("历史 Wi-Fi 名称", "⚠️ 未找到", os.path.basename(full_path)))

    # 2. 提取 MAC 地址 (BSSID 或 Randomized MAC)
    ...
    if valid_macs:
        ...
        results.append(("Wi-Fi MAC/BSSID", "⚠️ 未找到", os.path.basename(full_path)))
...
def main():
    print("==================================================")
    print("   Android 取证数据提取工具 (IMEI/IMSI/WiFi/号码)   ")
    print("==================================================\n")
